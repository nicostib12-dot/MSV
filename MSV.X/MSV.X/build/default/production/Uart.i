...
# 19 "Uart.c" 2
# 30 "Uart.c"
static volatile unsigned char tx_buf[32U];
...
void UART_TxISR(void) {
    if (tx_count > 0U) {
        ...
        tx_head = (unsigned char)((tx_head + 1U) % 32U);
        tx_count--;
    }
...
# 111 "Uart.c"
void __attribute__((picinterrupt(("")))) ISR_Global(void) {
...
void UART_SendChar(char c) {
    ...
    if (tx_count < (unsigned char)32U) {
        tx_buf[tx_tail] = (unsigned char)c;
        tx_tail = (unsigned char)((tx_tail + 1U) % 32U);
        tx_count++;
    }
...
void UART_SendVitals(unsigned int bpm, unsigned int temp_decimas) {
    ...
    UART_SendString("BPM:");
    if (bpm < 100U) UART_SendChar('0');
    if (bpm < 10U) UART_SendChar('0');
    UART_SendInt(bpm);
    ...
    UART_SendInt(temp_decimas / 10U);
    UART_SendChar('.');
    UART_SendChar((char)((temp_decimas % 10U) + (unsigned int)'0'));
