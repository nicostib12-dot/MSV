...
# 46 "Max30102.c" 2
# 71 "Max30102.c"
static unsigned short ir_buf[100U];




static unsigned char raw_buf[50U * 3U];
...
static void MAX30102_UpdateBuffer(void) {
    ...
    wr_ptr = I2C_ReadReg(0x57, 0x04);
    rd_ptr = I2C_ReadReg(0x57, 0x06);
    ...
    if (I2C_ReadBurst(0x57, 0x07,
                      raw_buf, (unsigned char)(num_samples * 3U)) != 0) {
        return;
    }



    if (samples_in_buf >= (unsigned char)100U) {
        for (i = 0; i < (unsigned char)(100U - num_samples); i++) {
            ir_buf[i] = ir_buf[i + num_samples];
        }
        samples_in_buf = (unsigned char)(100U - num_samples);
    }
# 146 "Max30102.c"
...
static unsigned int MAX30102_CalcBPM(void) {
    ...
    min_val = ir_buf[0];
    max_val = ir_buf[0];
    for (i = 1U; i < (unsigned char)100U; i++) {
    ...
    if ((unsigned short)(max_val - min_val) < (unsigned short)1000U) {
        return last_bpm;
    }
    ...
    for (i = 0U; i < (unsigned char)100U && peak_count < 8U; i++) {
    ...
    bpm = (unsigned int)((60UL * (unsigned long)50U)
...
unsigned char MAX30102_Init(void) {
    ...
    part_id = I2C_ReadReg(0x57, 0xFF);
    if (part_id != (unsigned char)0x15) {
        return 1;
    }
    ...
    if (I2C_WriteReg(0x57, 0x09, 0x40U) != 0) {
        return 2;
    }
    ...
    I2C_WriteReg(0x57, 0x02, 0x00U);
    I2C_WriteReg(0x57, 0x03, 0x00U);




    I2C_WriteReg(0x57, 0x04, 0x00U);
    I2C_WriteReg(0x57, 0x05, 0x00U);
    I2C_WriteReg(0x57, 0x06, 0x00U);
    ...
    I2C_WriteReg(0x57, 0x08, 0x1FU);




    I2C_WriteReg(0x57, 0x09, 0x02U);
    ...
    I2C_WriteReg(0x57, 0x0A, 0x23U);





    I2C_WriteReg(0x57, 0x0C, 0x24U);


    for (i = 0U; i < (unsigned char)100U; i++) {
        ir_buf[i] = 0U;
    }
    samples_in_buf = 0U;
    last_bpm = 0U;
