...
# 72 "Bme280.c"
    status = I2C_ReadBurst(0x76, 0x88, buf, 6);
...
unsigned char BME280_Init(void) {
    ...
    chip_id = I2C_ReadReg(0x76, 0xD0);
    if (chip_id != 0x60) return 1;




    status = I2C_WriteReg(0x76, 0xE0, 0xB6);
    if (status != 0) return 2;
    ...
    status = I2C_WriteReg(0x76, 0xF2, 0x00);
    if (status != 0) return 2;
    ...
    status = I2C_WriteReg(0x76, 0xF5, 0x00);
    if (status != 0) return 2;
    ...
    status = I2C_WriteReg(0x76, 0xF4, 0x20);
    if (status != 0) return 2;
...
unsigned int BME280_ReadTemp(void) {
    ...
    status = I2C_WriteReg(0x76, 0xF4, 0x21);
    if (status != 0) return last_temp_decimas;
    ...
    status = I2C_ReadBurst(0x76, 0xFA, buf, 3);
    if (status != 0) return last_temp_decimas;
